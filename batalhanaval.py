...
def obter_pontos_barco(nome):
    return pontos_barcos.get(nome[0], 0)
...
def atacar(campo_oponente, campo_visivel, row, col, pontuacao_jogador):
    if campo_visivel[row][col] != "_":
        ...
        return False, pontuacao_jogador
    ...
    if campo_oponente[row][col] != "_":
        ...
        if not any(barco_acertado == campo_oponente[r][c] for r in range(tamanho_mapa) for c in range(tamanho_mapa)):
            for nome, inicial in zip(nomes_barcos, [n[0] for n in nomes_barcos]):
                if inicial == barco_acertado:
                    pontos = obter_pontos_barco(nome)
                    pontuacao_jogador += pontos
                    print(f"Destruíste um {nome}! Ganhou {pontos} pontos!")
                    break
    else:
        print("Errou")
        campo_visivel[row][col] = "-"
    return True, pontuacao_jogador
        ...
        if campo_jogador[row][col] != "_":
            ...
            if not any(barco_acertado == campo_jogador[r][c] for r in range(tamanho_mapa) for c in range(tamanho_mapa)):
                for nome, inicial in zip(nomes_barcos, [n[0] for n in nomes_barcos]):
                    if inicial == barco_acertado:
                        pontos = obter_pontos_barco(nome)
                        pontuacao_computador += pontos
                        print(f"O Computador destruiu o teu {nome} e ganhou {pontos} pontos!")
                        break
        else:
            print("Computador errou!")
            campo_visivel_computador[row][col] = "-"
        break
    return pontuacao_computador
    ...
    while True:
        while True:
            try:
                ...
                ataque_valido, pontuacao_jogador = atacar(campo_computador, campo_visivel_jogador, row, col, pontuacao_jogador)
                if ataque_valido:
                    break
            except ValueError:
                print("Por favor, insere um inteiro válido.")
        ...
        pontuacao_computador = computador_ataca(campo_utilizador, campo_visivel_computador, pontuacao_computador)

        print("\nTabuleiro do jogador após o ataque do computador: ")
        mostrar_campo(campo_visivel_computador)
